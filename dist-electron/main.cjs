...
function startLocalProductionServer(distPath) {
  return new Promise((resolve, reject) => {
    staticServer = import_node_http.default.createServer((req, res) => {
      ...
      const filePath = import_node_path.default.join(distPath, reqPath);
      ...
      import_node_fs.default.readFile(filePath, (err, data) => {
        if (err) {
          import_node_fs.default.readFile(import_node_path.default.join(distPath, "index.html"), (fallbackErr, fallbackData) => {
            if (fallbackErr) {
              res.writeHead(404);
              res.end("Not Found");
            } else {
